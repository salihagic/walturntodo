...
class LocalizationBloc extends Bloc<LocalizationEvent, LocalizationState> {
  ...
  Future<void> _init(
      LocalizationInitEvent event, Emitter<LocalizationState> emit) async {
    String? languageCode = await storageRepository.get(AppKeys.language_code);
    final languagePreviouslySelected = languageCode != null;
    final locale = languagePreviouslySelected
        ? Locale(languageCode)
        : AppLanguages.values.first.locale;

    Intl.defaultLocale = locale.languageCode;
    restApiClient.setAcceptLanguageHeader(locale.languageCode);

    emit(state.copyWith(
        status: languagePreviouslySelected
            ? LocalizationStateStatus.initialized
            : LocalizationStateStatus.initializedDefault,
        locale: locale));
  }
